...
| 7 | School management (admin CRUD + haversine helper) | In progress | `src/lib/geofence.ts` (haversine + `checkGeofence`) done, unit tests pending. Seed script has 15 real schools with approximate municipal-center coordinates (NOT surveyed — placeholder until an admin corrects via the real map picker). CRUD UI/API not started. |
